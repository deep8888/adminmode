...
public class ProductDao {
	...
	//상품 추가!
		public boolean addProduct(Product product) {
			...
			String sqlStatement = "insert into estore.product (name, category, price, manufacturer, unitInstock, description) values(?,?,?,?,?,?)";
		...
		public boolean deleteProduct(int id) {
			...
			String sqlStatement = "delete from estore.product where id =?";
		...
		public Product getProductById(int id) {
			// db에서 쿼리를 진행할 sql문을 만들었다.
			//특정한 id를 가진 레코드 나와라
			String sqlStatement = "select * from estore.product where id =?";
			// 레코드 형태로 들어오는데 그거를 object 형식으로 바꿔줘야함.
			// 그걸 RowMapper의 mapRow함수가 해
		...
		//상품 업데이트
				public boolean updateProduct(Product product) {
					...
					String sqlStatement = "update estore.product "
